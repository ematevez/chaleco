...
class ChalecoApp(App):
    ...
    def show_popup(message, title="Información"):
        def create_popup(dt):
    ...
    def on_destruction_button(self, instance):
        global qr_last_seen
        if qr_last_seen:
            # Mostrar un popup que indica que la destrucción ha comenzado
            ...
            # Crear un hilo separado para realizar la operación de destrucción para no bloquear la interfaz
            def destroy_chaleco():
                # Simular tiempo de procesamiento (opcional)
                time.sleep(2)  # Simular que toma tiempo en destruir

                # Actualizar el chaleco en la base de datos
                update_chaleco_status(qr_last_seen, 1)

                # Cerrar el popup de "Comenzando destrucción..."
                popup.dismiss()

                # Mostrar un popup de éxito
                self.show_popup(f"Chaleco con QR {qr_last_seen} destruido.")

                # Actualizar la lista de chalecos
                self.update_chaleco_list()

                # Desactivar nuevamente el botón de destrucción hasta que se escanee otro QR
                self.destruction_button.disabled = True

            # Crear el hilo
            destruction_thread = threading.Thread(target=destroy_chaleco)
    ...
    def on_start_server(self, instance):
        ...
        scan_thread = threading.Thread(target=start_server(self))
        scan_thread.start()
...
def convert_base64_to_image(base64_string):
    ...
    while len(qr_image) % 4 != 0:
        qr_image += '='

    try:
        decoded_data = base64.b64decode(qr_image)
        return decoded_data
...
def start_server(app_instance):
    ...
    while server_running:
        try:
            ...
            print("Servidor detenido1.")
            # Mostrar un popup cuando se cierra el servidor
            show_popup_data("Conexión finalizada. El servidor ha sido detenido1.", title="Servidor detenido")
            client_socket.close()
