...
# Verificar si las clases de Bluetooth están disponibles correctamente
try:
    BluetoothAdapter = autoclass('android.bluetooth.BluetoothAdapter')
    BluetoothDevice = autoclass('android.bluetooth.BluetoothDevice')
    UUID = autoclass('java.util.UUID')
except Exception as e:
    print(f"Error al cargar clases de Bluetooth: {e}")
...
class MyBluetoothApp(App):
    ...
    def send_data_via_bluetooth(self, data):
        ...
        if not adapter:
            Clock.schedule_once(lambda dt: self.show_popup("Error", "El dispositivo no tiene soporte para Bluetooth"))
            return
        ...
        if not target_device:
            Clock.schedule_once(lambda dt: self.show_popup("Error", f"Dispositivo con MAC {target_mac_address} no encontrado"))
            return
        ...
        try:
            ...
            socket.connect()
            Clock.schedule_once(lambda dt: self.show_popup("Conectado", f"Conexión exitosa con {target_device.getName()} ({target_mac_address})"))
        except Exception as e:
            Clock.schedule_once(lambda dt: self.show_popup("Error", f"No se pudo conectar al dispositivo: {e}"))
            return
        ...
        try:
            ...
            output_stream.flush()
            Clock.schedule_once(lambda dt: self.show_popup("Éxito", "Datos enviados correctamente"))
        except Exception as e:
            Clock.schedule_once(lambda dt: self.show_popup("Error", f"Error al enviar datos: {e}"))
        finally:
            socket.close()
