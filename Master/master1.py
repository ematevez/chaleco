...
class MyBluetoothApp(App):
    def __init__(self, **kwargs):
        ...
        self.mac_address = "XX:XX:XX:XX:XX:XX"  # Dirección MAC del receptor Bluetooth
    ...
    def ver_registro(self, instance):
        ...
        for chaleco in chalecos:
            content += f"{chaleco[1]} | {chaleco[2]} | {chaleco[3]} | {chaleco[4]} | {chaleco[5]} | {chaleco[6]} | {chaleco[7]} | {chaleco[8]}\n"
    ...
    def start_send_data_thread(self, instance):
        threading.Thread(target=self.send_data_bluetooth, daemon=True).start()

    async def send_data(self, chaleco, client):
        lote = chaleco[1]
        numero_serie = chaleco[2]
        fabricante = chaleco[3]
        ...
        data_to_send = f"Lote: {lote}, Serie: {numero_serie}, Fabricante: {fabricante}, Fecha de Fabricación: {fecha_fabricacion}, Fecha de Vencimiento: {fecha_vencimiento}, Tipo: {tipo_modelo}, Peso: {peso}, Talla: {talla}, Procedencia: {procedencia}"
        await client.write_gatt_char("UUID", data_to_send.encode())  # "UUID" debe ser reemplazado por el UUID de la característica Bluetooth

    def send_data_bluetooth(self):
        chalecos = self.db.obtener_chalecos_no_transmitidos()
        ...
        async def run():
            try:
                ...
                for chaleco in chalecos:
                    await self.send_data(chaleco, client)
                    self.db.marcar_transmitido(chaleco[0])

                await client.disconnect()
            except Exception as e:
                print(f"Error enviando datos por Bluetooth: {e}")
